...
{
    ui->setupUi(this);
    std::unique_ptr<ApplicationManager> am_ptr = std::make_unique<ApplicationManager>();
...
void RunningWindow::indexAppStats(nlohmann::json &all_stats, QTableView *tableView)
{
    ...
    for (auto app_key = all_stats.begin(); app_key != all_stats.end(); app_key++){
